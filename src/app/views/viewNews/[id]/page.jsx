...
const ApprovedPostDetails = () => {
  ...
  useEffect(() => {
    const fetchPostDetails = async () => {
      if (id) {
        try {
          ...
          setPostDetails(response.data); // setState dentro de useEffect
        } catch (error) {
    ...
    fetchPostDetails();
  }, [id]); // Asegúrate de incluir `id` en las dependencias
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-white to-gray-50 rounded-3xl shadow-xl">
      ...
      <Modal
        open={isModalVisible} // Reemplazado por "open"
        footer={null}
        ...
        {postDetails.imagenes[selectedImageIndex] && (
            ...
            <Button
                ...
                downloadFileWithExtension(
                    postDetails.imagenes[selectedImageIndex].url,
                    `imagen_${selectedImageIndex + 1}`
                )
                }
